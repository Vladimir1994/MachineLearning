...
def HoltWintersAdditive(x, seasonPeriod, forecastLen):
    Y = x[:]
    initValues = array([0.5, 0.5, 0.5])
    boundaries = [(0, 1), (0, 1), (0, 1)]

    parameters = minimize(rmseHoltWintersAdditive, x0 = initValues, args = (Y, seasonPeriod),
        bounds = boundaries)
    alpha, beta, gamma = parameters.x

    level = [sum(Y[0:seasonPeriod]) / float(seasonPeriod)]
    trend = [(sum(Y[seasonPeriod:2 * seasonPeriod]) - sum(Y[0:seasonPeriod])) / seasonPeriod ** 2]
    ...
    for i in range(len(Y) + forecastLen):
        ...
        level.append(alpha * (Y[i] - season[i]) + (1 - alpha) * (level[i] + trend[i]))
        trend.append(beta * (level[i + 1] - level[i]) + (1 - beta) * trend[i])
        season.append(gamma * (Y[i] - level[i] - trend[i]) + (1 - gamma) * season[i])

...
def holt_winters_additive(x, season_period, forecast_len):
    y = x[:]
    init_values = array([0.5, 0.5, 0.5])
    boundaries = [(0, 1), (0, 1), (0, 1)]

    parameters = minimize(rmse_holt_winters_additive, x0=init_values,
...
def rmse_holt_winters_additive(params, *args):
    Y = args[0]
    ...
    trend = [(sum(y_train[season_period:2 * season_period]) -
              sum(Y[0:season_period])) / season_period ** 2]
    season = [y_train[i] - level[0] - trend[0] for i in range(season_period)]
    forecast = [level[0] + trend[0] + season[0]]
